...
const dateLocale = () => (document.documentElement.lang === 'ar' ? 'ar-u-nu-latn' : 'en-GB');

/* This file is shared by the demo apps and holds no strings of its own, so
   the relative-time wording is injected from the app's dictionary. The
   English defaults below are what `ago()` returns until it is. */
let AGO = {
  justNow: 'just now',
  m: (n) => `${n}m ago`,
  h: (n) => `${n}h ago`,
...
export const setRelativeStrings = (o) => { AGO = { ...AGO, ...(o || {}) }; };
...
/* Same reasoning as the relative-time strings: the two default button
   labels are injected by the app rather than living in this shared file. */
let DIALOG = { confirm: 'Confirm', cancel: 'Cancel' };
export const setDialogStrings = (o) => { DIALOG = { ...DIALOG, ...(o || {}) }; };

export function confirmDialog(message, { title = DIALOG.confirm, danger = false, okLabel = DIALOG.confirm, cancelLabel = DIALOG.cancel } = {}) {
