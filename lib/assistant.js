...
export class Assistant {
  constructor(cfg) {
    ...
    this.cfg.ui = Object.assign({
      ...
      close: 'Close',
      openName: (n) => `Open ${n}`,
      fabTitle: (n) => `${n} — ask about this workspace`,
  ...
  mount(root = document.body) {
    ...
    fab.type = 'button';
    fab.title = this.cfg.ui.fabTitle(this.cfg.name);
    fab.setAttribute('aria-label', this.cfg.ui.openName(this.cfg.name));
    ...
    host.innerHTML = `
      <section class="assist${docked ? ' assist--docked' : ''}" role="dialog" aria-label="${esc(this.cfg.name)}">
        <header class="assist__head">
          ...
          <button class="btn btn--ghost btn--icon" data-act="reset" title="${esc(this.cfg.ui.clear)}" aria-label="${esc(this.cfg.ui.clear)}">${ICON.reset}</button>
          ${docked ? '' : `<button class="btn btn--ghost btn--icon" data-act="close" title="${esc(this.cfg.ui.close)}" aria-label="${esc(this.cfg.ui.close)}">${ICON.close}</button>`}
        </header>
        <div class="assist__log" data-log></div>
        <div class="assist__suggest" data-suggest></div>
        <form class="assist__form" data-form>
          <input class="input" placeholder="${esc(this.cfg.ui.placeholder)}" aria-label="${esc(this.cfg.ui.placeholder)}" autocomplete="off" data-input>
          <button class="btn btn--primary btn--icon" type="submit" title="${esc(this.cfg.ui.send)}" aria-label="${esc(this.cfg.ui.send)}">${ICON.send}</button>
        </form>
  ...
  _push(who, html, meta) {
    ...
    el.innerHTML = `<span class="msg__av">${who === 'me' ? esc(this.cfg.ui.you) : esc(this.cfg.initials)}</span>
  ...
  async ask(q) {
    ...
    catch (err) { out = { text: this.cfg.ui.edgeOfData }; }
  ...
  _renderActions(body, actions) {
    ...
    actions.forEach((a, i) => {
      ...
      btn.addEventListener('click', async () => {
        ...
        btn.textContent = a.doingLabel || this.cfg.ui.working;
        let result;
        try { result = await a.run(); } catch (err) { result = { text: this.cfg.ui.actionFailed }; }
        if (typeof result === 'string') result = { text: result };
        row.remove();
        const done = this._push('bot', '', '');
        done.innerHTML = render(result.text || this.cfg.ui.done);
        if (result.table) done.innerHTML += tableHTML(result.table);
        done.innerHTML += `<div class="msg__meta">${esc(result.meta || this.cfg.ui.changeApplied)}</div>`;
  ...
  _route(q) {
    ...
    if (best) return best;
    return { answer: () => ({ text: pick(this.cfg.fallbacks), suggestions: this.cfg.suggestions }), trace: this.cfg.ui.searchedIndex };
  }
}
